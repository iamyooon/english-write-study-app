...
#### 5. 저학년용 Drag & Drop 미션 시스템 (✅ 기본 구현 완료, ⚠️ 확장 기능 미구현)
**설계 요구사항:**
...
**현재 상태:**
- ✅ `@dnd-kit/core`, `@dnd-kit/sortable`, `@dnd-kit/utilities` 라이브러리 설치 완료
- ✅ `WordCard.tsx` 컴포넌트 구현 (드래그 가능한 단어 카드)
- ✅ `DropZone.tsx` 컴포넌트 구현 (문장 빈칸 드롭 영역)
- ✅ `DragDropMission.tsx` 메인 미션 컴포넌트 구현
- ✅ `/mission/drag-drop` 페이지 구현
- ✅ `/api/study/drag-drop-submit` API 엔드포인트 구현
- ✅ 기본 테스트 코드 작성

**미구현 (확장 기능):**
- ❌ 미션 데이터베이스 연동 (현재는 샘플 데이터 사용)
  - `missions` 테이블에 drag & drop 미션 템플릿 저장
  - API로 동적 미션 로드
- ❌ 더 많은 미션 템플릿 추가
  - 다양한 문장 패턴 (인사, 감정 표현, 일상 표현 등)
  - 여러 빈칸이 있는 복잡한 문장
- ❌ 애니메이션 및 사운드 효과 추가
  - 드래그 중 시각적 피드백 강화
  - 정답/오답 시 축하/격려 사운드
...
**필요 작업:**
- [ ] `missions` 테이블에 drag & drop 미션 템플릿 저장 구조 설계
- [ ] `/api/mission/drag-drop` API 엔드포인트 생성 (레벨별 미션 조회)
- [ ] 다양한 미션 템플릿 추가 (10개 이상)
- [ ] 애니메이션 라이브러리 추가 (framer-motion 등)
- [ ] 사운드 효과 추가 (Web Audio API 또는 라이브러리)
- [ ] 모바일 터치 이벤트 최적화
- [ ] 반응형 디자인 개선

---

#### 6. Word Planet 탐험대 캐릭터 (완전 미구현)
**설계 요구사항:**
...
#### 9. FCM (Firebase Cloud Messaging) 연동 (완전 미구현)
**설계 요구사항:**
- 부모 푸시 알림 전용
- 미션 완료 시 부모 기기로 알림
- 부적절한 내용 감지 시 부모 알림
...
### Week 2 (게이미피케이션)
5. 🟡 저학년용 Drag & Drop 미션 확장 (데이터베이스 연동, 템플릿 추가, 애니메이션)
6. 🟡 Word Planet 캐릭터
7. 🟡 에너지 게이지 UI
8. 🔴 Free 버전 제한 로직 완성
...
## 📦 백업된 미션 데이터 복원 작업 (TBD)

### 백업 파일 정보
- **파일 위치**: `missions-backup-limit.json`
- **백업 시점**: 2026-01-29T07:33:09.242Z
- **백업 사유**: 학년별로 100개만 활성화하고 나머지 미션들을 비활성화하여 비용 절감 및 기본 커리큘럼 구성

### 백업된 미션 통계
백업 파일에는 학년별로 비활성화된 미션들이 저장되어 있습니다:
- **1학년**: 310개 미션 (비활성화됨)
- **2학년**: 131개 미션 (비활성화됨)
- **3학년**: 0개 (이미 16개만 활성화되어 있음)
- **4학년**: 305개 미션 (비활성화됨)
- **5학년**: 0개 (이미 38개만 활성화되어 있음)
- **6학년**: 데이터 없음

### 백업 파일 구조
```json
{
  "timestamp": "2026-01-29T07:33:09.242Z",
  "byGrade": {
    "1": [...],  // 1학년 비활성화된 미션 배열
    "2": [...],  // 2학년 비활성화된 미션 배열
    "4": [...]   // 4학년 비활성화된 미션 배열
...
- `id`: 미션 UUID
- `grade`: 학년 (1-6)
- `grade_level`: 학년 레벨 (`elementary_low`, `elementary_high`)
- `mission_data`: 미션 데이터 (JSONB)
  - `korean`: 한글 문장 (영어로 되어 있음, 번역 필요)
  - `sentence`: 영어 예시 문장
  - 기타 미션 메타데이터
- `is_active`: 활성화 상태 (false)
- `mission_type`: 미션 타입 (`drag_drop`, `keyboard`)

### 복원 작업 필요 사항

#### 1. 영어 문장 번역 작업
- 백업된 미션들의 `korean` 필드가 영어로 되어 있음
- OpenAI API를 사용하여 한글로 번역 필요
- 예상 비용: 약 4,000개 미션 × API 비용
- 예상 시간: 약 30분 (400ms 지연 기준)

#### 2. 복원 스크립트 작성 필요
- [ ] 백업 파일 읽기 스크립트 작성
- [ ] 영어 문장 번역 스크립트 작성 (기존 `translate-korean-fields.ts` 활용)
- [ ] 번역 완료 후 DB 업데이트 스크립트 작성
- [ ] `is_active = true`로 복원하는 스크립트 작성

#### 3. 복원 전략
- **옵션 1**: 모든 백업 미션을 번역하여 복원
  - 장점: 모든 미션 사용 가능
  - 단점: 높은 비용 및 시간 소요
- **옵션 2**: 필요 시 선택적으로 복원
  - 장점: 비용 절감
...
### 관련 스크립트
- `scripts/translate-korean-fields.ts`: 영어 문장을 한글로 번역하는 스크립트
- `scripts/limit-missions-per-grade.ts`: 학년별로 미션 수를 제한하는 스크립트
- `scripts/check-missions-status.ts`: 학년별 미션 상태 확인 스크립트

### 작업 예상 일정
- **우선순위**: 낮음 (현재 활성화된 미션으로 기본 커리큘럼 운영 가능)
- **예상 작업 시간**: 1-2일 (번역 + 테스트)
- **예상 비용**: OpenAI API 비용 (약 $5-10, 미션 수에 따라 변동)
