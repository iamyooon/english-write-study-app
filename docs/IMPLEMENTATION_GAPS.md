...
**현재 상태:**
- ❌ `profiles.energy` 필드는 있으나 기본값이 5 (설계: 100)
- ❌ `profiles.energy_last_charged` 컬럼 없음
- ❌ 자정 에너지 자동 충전 로직 없음
- ❌ 미션 제출 시 에너지 소비 로직 없음 (`/api/study/submit`)
- ❌ 작성 문장당 에너지 생산 로직 없음
- ❌ 에너지 부족 시 카운트다운 UI 없음
- ❌ 홈 화면 에너지 게이지 없음

**필요 작업:**
- [ ] `profiles` 테이블에 `energy_last_charged` 컬럼 추가 (마이그레이션)
- [ ] `profiles.energy` 기본값을 100으로 변경 (설계에 따르면)
- [ ] 자정 에너지 자동 충전 로직 (Supabase Edge Function 또는 Cron Job)
- [ ] `/api/study/submit`에 에너지 소비 로직 추가 (미션 1회당 100 소비)
- [ ] 에너지 생산 로직 추가 (작성 문장당 +10)
- [ ] 에너지 부족 시 카운트다운 UI 컴포넌트
- [ ] 홈 화면(`app/page.tsx`)에 에너지 게이지 추가
