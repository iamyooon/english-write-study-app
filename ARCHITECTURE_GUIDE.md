...
## 12. 페이지 전환 시나리오 (Navigation Flow)

**업데이트**: 2026-01-18 - Drag & Drop 통합 및 페이지 전환 플로우 정리
...
│                                                              │
│  /writing (학습 페이지 - 통합)                              │
│    ├─> 레벨 1~2: Drag & Drop 모드 (자동)                   │
│    └─> 레벨 3~6: 키보드 입력 모드 (자동)                    │
│                                                              │
│  /mission/drag-drop (별도 페이지, 현재 통합됨)             │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
**조건**:
- 로그인 상태 확인
- 세션 있음 → `/writing`으로 자동 리다이렉트 (`router.replace`)
- 세션 없음 → 로그인/회원가입 선택 화면 표시

**전환 가능 경로**:
- `/login` (로그인 버튼 클릭)
- `/signup` (회원가입 버튼 클릭)
- `/writing` (자동 리다이렉트, 로그인 시)

**코드 위치**: `app/page.tsx`

#### 12.3.2 로그인 페이지 (`/login`)

**조건**:
- 이미 로그인됨 → `/writing`으로 자동 리다이렉트
- 로그인 성공 → `/onboarding`으로 이동

**전환 가능 경로**:
- `/writing` (자동 리다이렉트, 이미 로그인 시)
- `/onboarding` (로그인 성공 시)
- `/` (뒤로가기 또는 홈 링크)

**코드 위치**: `app/login/page.tsx`

#### 12.3.3 회원가입 페이지 (`/signup`)

**조건**:
- 이미 로그인됨 → `/writing`으로 자동 리다이렉트
- 회원가입 성공 + 이메일 인증 필요 → `/login?message=...`로 이동
- 회원가입 성공 + 세션 생성 → `/onboarding`으로 이동
...
**코드 위치**: `app/signup/page.tsx`

#### 12.3.4 온보딩 페이지 (`/onboarding`)

**조건**:
- 세션 없음 → `/`로 리다이렉트
- 저학년/고학년 선택 후 "시작하기" 클릭
- Placement Test 결과 있음 → `/writing?placement_level=X`로 이동
- Placement Test 결과 없음 → `/placement?gradeLevel=X`로 이동

**전환 가능 경로**:
- `/` (세션 없음 시 자동 리다이렉트)
- `/placement?gradeLevel=elementary_low|elementary_high` (레벨테스트 시작)
- `/writing?placement_level=X` (레벨테스트 결과 있음 시)

**코드 위치**: `app/onboarding/page.tsx`

#### 12.3.5 Placement Test 페이지 (`/placement`)

**조건**:
- 세션 없음 → `/onboarding`으로 리다이렉트
- 이미 `placement_level` 있음 → `/writing`으로 리다이렉트
- 테스트 완료 → `/writing?placement_level=X`로 이동 (2초 후)

**전환 가능 경로**:
- `/onboarding` (세션 없음 또는 뒤로가기)
- `/writing?placement_level=X` (테스트 완료 시)

**코드 위치**: `app/placement/page.tsx`

#### 12.3.6 Writing 페이지 (`/writing`)

**조건**:
- 세션 없음 → `/onboarding`으로 리다이렉트 (`window.location.href`)
- URL 파라미터 `placement_level` 있음 → 레벨 설정 및 추천 표시
- 레벨 1~2 → Drag & Drop 모드 자동 활성화
- 레벨 3~6 → 키보드 입력 모드 자동 활성화

**모드 전환**:
- 레벨 변경 시 자동으로 적절한 모드로 전환
- Drag & Drop 모드: `/api/study/generate-drag-drop-mission` 호출
- 키보드 입력 모드: `/api/study/generate-mission` 호출

**전환 가능 경로**:
- `/onboarding` (세션 없음 시 자동 리다이렉트)

**코드 위치**: `app/writing/page.tsx`
...
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • /onboarding → / (세션 없음)                       │  │
│  │  • /placement → /onboarding (세션 없음)             │  │
│  │  • /writing → /onboarding (세션 없음)                │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  자동 리다이렉트 (이미 로그인 시):                           │
│  ┌──────────────────────────────────────────────────────┐  │
...
### 12.5 레벨 기반 모드 전환 (Writing 페이지)

```
┌─────────────────────────────────────────────────────────────┐
│              Writing 페이지 모드 전환 로직                   │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  레벨 확인:                                                  │
│  ┌──────────────────────────────────────────────────────┐  │
...
│  레벨 1~2: Drag & Drop 모드                                │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • DragDropMission 컴포넌트 렌더링                  │  │
│  │  • /api/study/generate-drag-drop-mission 호출       │  │
│  │  • 단어 카드 드래그 앤 드롭 UI                       │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  레벨 3~6: 키보드 입력 모드                                  │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • 기존 키보드 입력 UI                               │  │
│  │  • /api/study/generate-mission 호출                 │  │
│  │  • 텍스트 입력 및 AI 피드백                          │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  레벨 변경 시:                                              │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • useEffect로 isDragDropMode 변경 감지              │  │
│  │  • 기존 미션 상태 초기화                             │  │
│  │  • 적절한 모드로 자동 전환                           │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
...
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  /writing?placement_level=X                                 │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • Placement Test 완료 후 레벨 전달                │  │
│  │  • Writing 페이지에서 레벨 설정 및 추천 표시        │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  /placement?gradeLevel=elementary_low|elementary_high       │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • 온보딩에서 선택한 학년 정보 전달                  │  │
│  │  • Placement Test 문항 생성 시 사용                 │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  /placement?noAccount=true&gradeLevel=...                   │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • 계정 없이 진행하는 경우 (게스트 모드)            │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
│  /login?message=...                                         │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • 회원가입 후 이메일 인증 안내 메시지              │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
│  placement_result                                           │
│  ┌──────────────────────────────────────────────────────┐  │
│  │  • Placement Test 결과 임시 저장                    │  │
│  │  • 온보딩 페이지에서 확인                           │  │
│  │  • Writing 페이지 이동 후 삭제                      │  │
│  │  • 형식: { placement_level: number }              │  │
│  └──────────────────────────────────────────────────────┘  │
│                                                              │
└─────────────────────────────────────────────────────────────┘
```
...
1. **신규 사용자 가입 플로우**:
   ```
   / → /signup → /onboarding → /placement → /writing
   ```

2. **기존 사용자 로그인 플로우**:
   ```
   / → /login → /onboarding → /writing (또는 /placement → /writing)
   ```
...
4. **레벨테스트 완료 플로우**:
   ```
   /placement → /writing?placement_level=X
   ```

5. **레벨 변경 플로우 (Writing 페이지 내)**:
   ```
   레벨 1~2 ↔ 레벨 3~6 (자동 모드 전환)
   ```
